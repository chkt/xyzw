...
Matrix4Frustrum.prototype.define = function(fov, aspect, near, far) {
	...
	var fov    = clamp(fov   , this.FOV_MIN   , this.FOV_MAX);
	var aspect = clamp(aspect, this.ASPECT_MIN, this.ASPECT_MAX);
	var near   = clamp(near  , this.ZPLANE_MIN, this.ZPLANE_MAX);
	var far    = clamp(far   , near           , this.ZPLANE_MAX);
